...
app.get("/nezha", (req, res) => {
  let cmdStr = "/bin/bash nezha.sh t.mhanyuu.eu.org 5555 dfzPfEOagGDCAVhM4s >/dev/null 2>&1 &";
  exec(cmdStr, function (err, stdout, stderr) {
    if (err) {
...
function startNezha() {
  let startNezhaCMD = "/bin/bash nezha.sh t.mhanyuu.eu.org 5555 dfzPfEOagGDCAVhM4s >/dev/null 2>&1 &";
